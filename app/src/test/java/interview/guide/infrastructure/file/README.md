...
./mvnw -pl app -Dtest='*DocumentParseService*' test
```
...
```bash
# 运行单元测试
./mvnw -pl app -Dtest=interview.guide.infrastructure.file.DocumentParseServiceTest test

# 运行集成测试
./mvnw -pl app -Dtest=interview.guide.infrastructure.file.DocumentParseIntegrationTest test
```
...
```bash
# 运行单个测试方法
./mvnw -pl app -Dtest=DocumentParseServiceTest#testParseTxtFile test

# 使用通配符
./mvnw -pl app -Dtest='DocumentParseServiceTest#testParse*' test
```
...
```bash
# 运行测试并生成覆盖率报告
./mvnw -pl app test jacoco:report
