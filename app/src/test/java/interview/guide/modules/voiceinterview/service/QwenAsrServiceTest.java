...
class QwenAsrServiceTest {
    ...
    void setUp() {
        ...
        VoiceInterviewProperties.AsrConfig asrConfig = properties.getQwen().getAsr();
        asrConfig.setUrl("wss://dashscope.aliyuncs.com/api-ws/v1/realtime");
        asrConfig.setModel("qwen3-asr-flash-realtime");
        asrConfig.setApiKey("test-api-key");
        asrConfig.setLanguage("zh");
        asrConfig.setFormat("pcm");
        asrConfig.setSampleRate(16000);
        asrConfig.setEnableTurnDetection(true);
        asrConfig.setTurnDetectionType("server_vad");
        asrConfig.setTurnDetectionThreshold(0.0f);
        asrConfig.setTurnDetectionSilenceDurationMs(400);
