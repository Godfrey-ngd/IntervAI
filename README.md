...
3. 构建工具为什么选择 Maven？主要考虑生态稳定、上手成本低，且更方便在 CI 与企业环境中统一配置。
...
若使用本仓库的 **Docker 只启动依赖**（PostgreSQL / Redis / MinIO），可先执行 `docker compose up -d postgres redis minio createbuckets`，将根目录 **`.env.example` 复制为 `.env`** 并填写 `AI_BAILIAN_API_KEY`，再运行后端；默认账号与 `docker-compose.yml` 一致。需要显式启用 `application-dev.yml` 时，可使用 `./mvnw -pl app spring-boot:run -Dspring-boot.run.profiles=dev`。
...
**本项目已做的配置**（一般无需再改）：`app/src/main/resources/logback-spring.xml`（控制台日志 UTF-8）、`app/pom.xml` 中 `spring-boot-maven-plugin` 的 JVM 参数（含 `file.encoding` / `stdout.encoding` / `stderr.encoding`）。

**仍乱码时（PowerShell 侧）**：在启动 `./mvnw.cmd -pl app spring-boot:run` 的同一终端先执行下面一段；或写入 **PowerShell 配置文件**（`$PROFILE`）以便每次自动生效：
...
在 PowerShell 中建议使用 `.\mvnw.cmd -pl app spring-boot:run`（或仓库根目录的 `.\mvnw.cmd`），避免与执行策略、路径解析相关的问题。
